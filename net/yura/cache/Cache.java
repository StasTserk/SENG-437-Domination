...
public class Cache {
	...
	// as this class is only used on j2se and android, we should use proper
	// logging
	...
	public Cache(String appName) {
		...
		if (DEBUG)
			logger.log(Level.INFO, "starting {0}", cacheDir);
		...
		if (file.exists()) {
			...
			try {
				...
				if (!cacheDir.isDirectory()) {
					...
					if (!cacheDir.mkdirs()) {
						throw new RuntimeException("can not make cache dir: "
								+ cacheDir);
					}
				}
				...
				logger.log(
						Level.WARNING,
						"failed to save data to file: " + file + " exists="
								+ exists + " deleted=" + deleted + " key: "
								+ key + " in dir " + cacheDir + " exists="
								+ cacheDir.exists() + " is dir"
								+ cacheDir.isDirectory(), ex);
			}
		}
	}
	...
	public InputStream get(String key) {
		...
		if (file.exists()) {
			try {
				...
				if (DEBUG)
					logger.log(Level.INFO, "getting from cache: {0}", key);
			...
			if (DEBUG)
				logger.log(Level.INFO, "key not found: {0}", key);
		}
		return null;
	}
