...
/**
 * Translation services for maps: Countries and continents
 * 
...
public class MapTranslator {
	...
	/**
	 * sets the currently used map Tries to find a matching properties file +
	 * resource bundle
	 * 
	 * @param strFile
	 *            The map file absolute path
	 */
	public static void setMap(String strFile) {
		...
		strFile = strName + "_"
				+ TranslationBundle.getBundle().getLocale().getLanguage()
				+ ".properties";

		// file exists, use it!
		try {
			MapResb = new PropertyResourceBundle(
					RiskUtil.openMapStream(strFile));

		} catch (Exception ioe) {
			try {
				MapResb = ResourceBundle.getBundle(
						"net.yura.domination.engine.translation.DefaultMaps",
	...
	/**
	 * returns the translation for the given string if any If there is no
	 * translation, the string is simply returned
	 */
	...
	public static void setCards(String INstrFile) {
		...
		try {
			...
			CardsResb = new PropertyResourceBundle(
					RiskUtil.openMapStream(strFile));

		} catch (Exception ioe) {

			if (INstrFile.equals("risk.cards")) { // only use this with the
													// default cards file
				// load default cards translation bundle
				try {
					CardsResb = ResourceBundle
							.getBundle(
									"net.yura.domination.engine.translation.DefaultCards",
									TranslationBundle.getBundle().getLocale());
				} catch (MissingResourceException e) {
