...
public class Risk extends Thread {
	...
	public static final String[] types = new String[] { "human", "ai easy",
			"ai easy", "ai easy", "ai average", "ai average" };
	public static final String[] names = new String[] { "player", "bob",
			"fred", "ted", "yura", "lala" };
	public static final String[] colors = new String[] { "cyan", "green",
	...
	public Risk() {
		...
		for (int c = 0; c < names.length; c++) {
			riskconfig.setProperty("default.player" + (c + 1) + ".type",
					types[c]);
			riskconfig.setProperty("default.player" + (c + 1) + ".color",
					colors[c]);
			riskconfig.setProperty("default.player" + (c + 1) + ".name",
					names[c]);
		}
		...
		RiskGame.setDefaultMapAndCards(riskconfig.getProperty("default.map"),
				riskconfig.getProperty("default.cards"));
	...
	static String createRandomUniqueAddress() {
		String randomString = "#"
				+ String.valueOf(Math.round(Math.random() * Long.MAX_VALUE));
		try {
			try {
	...
	/**
	 * This parses the string, calls the relavant method and displays the
	...
	public void parserAndWait(String uiCommand) throws InterruptedException {
		GameCommand gCommand = new GameCommand(GameCommand.UI_COMMAND,
				uiCommand);
	...
	private void processFromUI(String message) {
		...
		if (message.equals("ver")) {
			...
			controller.sendMessage(RiskUtil.GAME_NAME
					+ " Game Engine [Version " + RISK_VERSION + "]", false,
					false);
			getInput();
		}
		// take no action
					...
					if (unlimitedLocalMode) {
						...
						try {
							...
							logger.log(Level.WARNING,
									"error saving game to file: " + filename,
									ex);
				...
				if (StringT.hasMoreTokens() == false) {
					if (unlimitedLocalMode) {
						try {
							...
							game = new RiskGame();
							replay = true;
							for (Iterator e = replayCommands.iterator(); e
									.hasNext();) {
	...
	private void noGameParser(String message) {
		...
		if (input.equals("newgame")) {
			...
			if (StringT.hasMoreTokens() == false) {
				...
				try {
					...
					RiskUtil.printStackTrace(e);
					output = resb.getString("core.newgame.error") + " "
							+ e.toString();
				}
				// }
				// else {
				...
				output = RiskUtil.replaceAll(
						resb.getString("core.error.syntax"), "{0}", "newgame");
			}
		...
		// LOAD GAME
			...
			if (StringT.countTokens() >= 1) {
				...
				// this is not needed here as u can only get into this bit of
				// code if game == null
				// if (game == null) {
				String filename = StringT.nextToken();
				...
				try {
					...
					if (player != null) {
						...
						output = output + System.getProperty("line.separator")
								+ resb.getString("core.loadgame.currentplayer")
								+ " " + player.getName();
					}
					...
					logger.log(Level.WARNING, "error loading game from file: "
							+ filename, ex);

					output = resb.getString("core.loadgame.error.load") + " "
							+ ex;
					showMessageDialog(output);
				}
				// }
			...
			if (StringT.countTokens() == 1) {
				...
				try {
					...
					onlinePlayClient = new ChatClient(this, myAddress,
							StringT.nextToken(), port);
				...
				} catch (Exception e) { // catch not being able to make a new
										// game, so game is null
			...
			if (StringT.hasMoreTokens() == false) {
				...
				if (p2pServer != null) {
					...
					try {
						...
						output = resb.getString("core.killserver.error") + " "
								+ e.getMessage();
					}
	...
	protected void inGameParser(final String message) {
		...
		if (Addr.equals("ERROR")) { // server has sent us a error
			...
			// @todo, do we set needinput to flase, so that ai wont go twice, if
			// its there go
			// but then if there needinput was ignored coz this command was in
			// the inbox, no needinput will get called

			// if it is NOT there go the set needinput to false
			if (game.getCurrentPlayer() != null
					&& !(game.getCurrentPlayer().getAddress().equals(id) && game
							.getCurrentPlayer().getType() == Player.PLAYER_HUMAN)) {

				needInput = false;

			}
			// if this command stopped the last needInput from being called,
			// then this will be screwed
			// at worst AI or human wont get a chance to put any input in, game
			// stalled

			output = "someone has gone: ";

			// get all the players and make all with the ip of the leaver become
			// nutral
			...
			output = resb.getString("core.dice.rolling")
					+ System.getProperty("line.separator")
					+ resb.getString("core.dice.results");
			...
			if (result[0] == 1) {
				output = output
						+ RiskUtil.replaceAll(RiskUtil.replaceAll(
								resb.getString("core.dice.result"), "{0}",
								String.valueOf(result[2])) // defeated
				...
				if (result[3] == 0) {
					...
					output = output + System.getProperty("line.separator")
							+ resb.getString("core.dice.notdefeated") + " ";
					...
					if (n > 0) {
						...
						// ((Country)game.getAttacker()).getOwner();
						//
						...
						output = output
								+ resb.getString("core.dice.noattackagain");
					}
					...
					//
					// if ( showHumanPlayerThereInfo( attackingPlayer ) ) {
					// controller.setSlider( game.getMustMove(),
					// game.getAttacker().getColor(),
					// game.getDefender().getColor() );
					// }

					output = output + System.getProperty("line.separator")
							+ resb.getString("core.dice.defeated") + " ";

					if (result[3] == 2) {
						output = output
								+ resb.getString("core.dice.eliminated") + " ";
					}
					...
					if (result[4] == result[5]) {
						...
						output = output
								+ RiskUtil.replaceAll(RiskUtil.replaceAll(resb
										.getString("core.dice.howmanyarmies"),
				...
				output = resb.getString("core.dice.error.unabletoroll");
			}

			// ==1 this fixes the automove bug, when u need to trade after
			// rolling and automove
			if (game.getState() != RiskGame.STATE_ROLLING
					&& game.getState() != RiskGame.STATE_DEFEND_YOURSELF) {
			...
			output = RiskUtil.replaceAll(
					resb.getString("core.player.randomselected"), "{0}",
					p.getName());
			...
			output = RiskUtil.replaceAll(
					resb.getString("core.player.newselected"), "{0}",
					newplayer.getName());

			// this is not a bug! (Easter egg)
			if (unlimitedLocalMode && game.getSetupDone()
					&& newplayer.getName().equals("Theo")) {
			...
			Country c = game
					.getCountryInt(Integer.parseInt(StringT.nextToken()));
			game.placeArmy(c, 1);
			controller.sendMessage(
					RiskUtil.replaceAll(
							resb.getString("core.place.oneplacedin"), "{0}",
							c.getName()), false, false); // Display
			...
			for (int c = 0; c < game.getNoCountries(); c++) {
				...
				Country t = game.getCountryInt(Integer.parseInt(StringT
						.nextToken()));
				game.placeArmy(t, 1);
				controller.sendMessage(RiskUtil.replaceAll(RiskUtil.replaceAll(
						resb.getString("core.place.getcountry"), "{0}",
						((Player) game.getCurrentPlayer()).getName()), "{1}", t
						.getName()) // Display
						, false, false);
				game.endGo();
			}
			 ...
			 * );
			 * 
			 * String oldName = (String)map.get("oldName"); String newName =
			 * (String)map.get("newName"); String newAddress =
			 * (String)map.get("newAddress"); int newType =
			 * Integer.parseInt((String)map.get("newType"));
			 * 
			 * try { renamePlayer(oldName,newName,newAddress,newType); } catch
			 * (Exception ex) { ex.printStackTrace(); }
			 */
			System.err.println("Tried to execute missing URL code");
			...
			if (game != null && game.getCurrentPlayer() != null
					&& game.getState() != RiskGame.STATE_GAME_OVER) {
				...
				if (type == Player.PLAYER_HUMAN) {
					...
					key = "newgame.player.type." + ai.getCommandFromType(type)
							+ "ai";
				}
				...
				controller.sendMessage(game.getCurrentPlayer().getName() + "("
						+ typeString + ")>" + echo, false, false);
			} else {
				...
				if (input.equals("choosemap")) {
					...
					if (StringT.countTokens() >= 1) {
						...
						try {
							...
							// crap, we wanted to use this map, but we would not
							// load it
							// maybe we can download it from the server and then
							// use it
					...
					if (StringT.countTokens() >= 1) {
						...
						try {
							...
							controller.showCardsFile(game.getCardsFile(),
									yesmissions);
							// New cards file selected: "{0}"
							output = RiskUtil.replaceAll(
									resb.getString("core.choosecards.chosen"),
									"{0}", filename);
						} catch (Exception e) {
							output = resb
									.getString("core.choosecards.error.unable");
						}
					} else {
						output = RiskUtil.replaceAll(
								resb.getString("core.error.syntax"), "{0}",
								"choosecards filename");
					}
					...
					if (StringT.countTokens() >= 3) {
						...
						if (color != 0
								&& t != -1
								&& !name.equals("")
								&& ((unlimitedLocalMode && game.addPlayer(t,
										name, color, "LOCALGAME")) || (!unlimitedLocalMode && game
										.addPlayer(t, name, color, Addr)))) {
							...
							output = resb
									.getString("core.newplayer.error.unable");
						}

					} else {
						output = RiskUtil.replaceAll(
								resb.getString("core.error.syntax"), "{0}",
								"newplayer type (skill) color name");
					}
					...
					if (StringT.countTokens() >= 1) {
						...
						if (game.delPlayer(name)) {
							...
							output = RiskUtil.replaceAll(
									resb.getString("core.delplayer.deleted"),
									"{0}", name);
						} else {
							output = resb
									.getString("core.delplayer.error.unable");
						}
					} else {
						output = RiskUtil.replaceAll(
								resb.getString("core.error.syntax"), "{0}",
					...
					if (StringT.hasMoreTokens() == false) {
						...
						for (int a = 0; a < players.size(); a++) {
							...
							output = output
									+ resb.getString("core.info.player") + " "
									+ ((Player) players.get(a)).getName()
									+ "\n";

						}

						output = output + resb.getString("core.info.mapfile")
								+ " " + game.getMapFile() + "\n";
						output = output + resb.getString("core.info.cardsfile")
								+ " " + game.getCardsFile();
					...
					if (StringT.hasMoreTokens() == false) {
						...
						if (game.getPlayers().size() == 0) {
							...
							output = resb
									.getString("core.info.autosetup.error");
					...
					if (StringT.countTokens() >= 2
						...
						if (crap == null) {
							// checks all the options are correct to start a
							// game
							if (newgame_type != -1 && newgame_cardType != -1
									&& n >= 2 && n <= RiskGame.MAX_PLAYERS) {

								autoplaceall = newgame_autoplaceall;

								try {

									game.startGame(newgame_type,
											newgame_cardType, newgame_recycle,
											threeDice);
							...
							if (game.getState() != RiskGame.STATE_NEW_GAME) {
								...
								if (shouldGameCommand(Addr)) {
									...
									if (game.getGameMode() == RiskGame.MODE_SECRET_MISSION
										...
										for (int c = 0; c < a.size(); c++) {
											...
											outputb.append(((Country) a.get(c))
													.getColor());
										}

										gameCommand(Addr, "PLACEALL",
												outputb.toString());
									}
								...
								output = resb
										.getString("core.start.error.players");
							}
						} else {
						...
						output = RiskUtil
								.replaceAll(
										resb.getString("core.error.syntax"),
										"{0}",
										"startgame gametype cardtype (autoplaceall recycle)");
					}
				}
				...
				// REPLAY A GAME
					...
					if (StringT.countTokens() >= 1) {
						...
						try {
							...
							// TODO dont think this can ever work as an applet
							// anyway
							// if (Risk.applet==null) {
							...
							// else {
							// url = new URL(
							// net.yura.domination.engine.Risk.applet.getCodeBase()
							// , filename );
							// }
					...
					output = RiskUtil
							.replaceAll(resb.getString("core.error.incorrect"),
									"{0}",
									"newplayer, delplayer, startgame, choosemap, choosecards, info, autosetup");
				}
					...
					if (input.equals("undo")) {
						...
						if (StringT.hasMoreTokens() == false) {
							...
							if (unlimitedLocalMode) {
								...
								try {
									// can not undo when defending yourself as
									// it is not really your go
									if (game.getState() != RiskGame.STATE_DEFEND_YOURSELF
											&& Undo != null && Undo.size() != 0) {
										// game = (RiskGame)Undo.getObject(
										// nullCipher );
									...
									logger.log(
											Level.WARNING,
											resb.getString("core.loadgame.error.undo"),
											e);
								}
							} else {
								output = resb
										.getString("core.undo.error.network");
							}
						...
						if (StringT.hasMoreTokens() == false) {
							...
							if (showHumanPlayerThereInfo()) {
								output = resb
										.getString("core.showmission.mission")
										+ " " + getCurrentMission();
							} else {
								output = resb
										.getString("core.showmission.error");
							}
						} else {
							output = RiskUtil.replaceAll(
									resb.getString("core.error.syntax"), "{0}",
						...
						if (StringT.hasMoreTokens() == false) {
							...
							if (game.getState() != RiskGame.STATE_NEW_GAME) {
								...
								output = resb
										.getString("core.showarmies.countries")
										+ System.getProperty("line.separator");

								for (int c = 0; c < v.length; c++) {

									output = output + v[c].getColor() + " "
											+ v[c].getName() + " - "; // Display
									...
									if (v[c].getOwner() != null) {
										...
										if (game.getGameMode() == 2
												&& game.getSetupDone()
												&& game.getState() != RiskGame.STATE_SELECT_CAPITAL) {

											List players = game.getPlayers();

											for (int a = 0; a < players.size(); a++) {

												if (((Player) players.get(a))
														.getCapital() != null
														&& ((Player) players
																.get(a))
																.getCapital() == v[c]) {
										...
										output = output
												+ resb.getString("core.showarmies.noowner")
												+ System.getProperty("line.separator");
									}
								...
								output = resb
										.getString("core.showarmies.error.unable");
							}
						} else {
							output = RiskUtil.replaceAll(
									resb.getString("core.error.syntax"), "{0}",
						...
						if (StringT.hasMoreTokens() == false) {
							...
							if (showHumanPlayerThereInfo()) {
								...
								List c = ((Player) game.getCurrentPlayer())
										.getCards();

								if (c.size() == 0) {
									output = resb
											.getString("core.showcards.nocards");
								} else {
									output = resb
											.getString("core.showcards.youhave");

									for (int a = 0; a < c.size(); a++) {

										if (((Card) c.get(a)).getName().equals(
												Card.WILDCARD)) {
											output = output + " "
													+ Card.WILDCARD; // resb.getString(
								...
								if (game.getCardMode() == RiskGame.CARD_FIXED_SET) {
									...
									output = output
											...
											+ RiskUtil
													.replaceAll(
															resb.getString("cards.nexttrade"),
															"{0}",
															String.valueOf(getNewCardState()));
								}

							} else {
								output = resb
										.getString("core.showcards.error.unable");
							}
						} else {
							output = RiskUtil.replaceAll(
									resb.getString("core.error.syntax"), "{0}",
						...
						if (StringT.hasMoreTokens() == false) {
							...
							output = RiskUtil.replaceAll(
									resb.getString("core.autoendgo.setto"),
									"{0}", resb.getString(strSelected));
							...
							if (option.equals("on")) {
								...
								output = RiskUtil.replaceAll(
										resb.getString("core.autoendgo.setto"),
										"{0}",
										resb.getString("core.autoendgo.on"));
							} else if (option.equals("off")) {
								game.getCurrentPlayer().setAutoEndGo(false);
								output = RiskUtil.replaceAll(
										resb.getString("core.autoendgo.setto"),
										"{0}",
										resb.getString("core.autoendgo.off"));
							} else {
								output = RiskUtil
										.replaceAll(
												resb.getString("core.autoendgo.error.unknown"),
												"{0}", option);
							}
						...
						if (StringT.hasMoreTokens() == false) {
							...
							output = RiskUtil.replaceAll(
									resb.getString("core.autodefend.setto"),
									"{0}", resb.getString(strSelected));
						} else if (StringT.countTokens() == 1) {
							...
							if (option.equals("on")) {
								...
								output = RiskUtil
										.replaceAll(
												resb.getString("core.autodefend.setto"),
												"{0}",
												resb.getString("core.autodefend.on"));
							} else if (option.equals("off")) {
								...
								output = RiskUtil
										.replaceAll(
												resb.getString("core.autodefend.setto"),
												"{0}",
												resb.getString("core.autodefend.off"));
							} else {
								output = RiskUtil
										.replaceAll(
												resb.getString("core.autodefend.error.unknown"),
												"{0}", option);
							}
						...
						if (input.equals("trade")) {
							if (StringT.countTokens() == 3) {
								...
								if (cards[0] != null && cards[1] != null
										&& cards[2] != null) { // if the player
																// DOES HAVE all
																// the cards he
																// chose
									noa = game.trade(cards[0], cards[1],
											cards[2]);
								}
								...
								if (noa != 0) { // if the trade WAS SUCCESSFUL
									output = RiskUtil
											.replaceAll(
													resb.getString("core.trade.traded"),
													"{0}", "" + noa);
								} else {
									throw new IllegalArgumentException(
											resb.getString("core.trade.error.unable"));
								}
							} else {
								throw new IllegalArgumentException(
										RiskUtil.replaceAll(
												resb.getString("core.error.syntax"),
												"{0}", "trade card card card"));
							}
							...
							if (StringT.hasMoreTokens() == false) {
								...
								if (game.endTrade()) {
									output = resb
											.getString("core.trade.endtrade");
								} else {
									throw new IllegalArgumentException(
											resb.getString("core.trade.end.error.unable"));
								}
							} else {
								throw new IllegalArgumentException(
										RiskUtil.replaceAll(
							...
							throw new IllegalArgumentException(
									RiskUtil.replaceAll(
											resb.getString("core.error.incorrect"),
											"{0}",
											"showcards, trade"
													+ (game.canEndTrade() ? ", endtrade"
															: "")));
						}
						...
						if (input.equals("placearmies")) {
							if (StringT.countTokens() == 2) {
								...
								int num = RiskGame.getNumber(StringT
										.nextToken());
								Country t;
								...
								if (c != -1) {
									...
									// YURA:LANG
									// t=game.getCountryByName(country);
									t = null;
								}

								if (t != null
										&& num != -1
										&& !(game.getGameMode() == 1 && t
												.getOwner() == null)
										&& !(game.getGameMode() == 3 && t
												.getOwner() == null)) {
									...
									if (result != 0) {
										...
										throw new IllegalArgumentException(
												resb.getString("core.place.error.unable"));
									}
								} else {
									throw new IllegalArgumentException(
											resb.getString("core.place.error.invalid"));
								}
							} else {
								throw new IllegalArgumentException(
										RiskUtil.replaceAll(
							...
							throw new IllegalArgumentException(
									RiskUtil.replaceAll(resb
											.getString("core.error.incorrect"),
											"{0}",
						...
						if (input.equals("attack")) {
							if (StringT.countTokens() == 2) {
								...
								if (a1 != -1) {
									...
									// YURA:LANG
									// country1=game.getCountryByName(arg1);
									country1 = null;
								}
								...
								if (a2 != -1) {
									...
									// YURA:LANG
									// country2=game.getCountryByName(arg2);
									country2 = null;
								}
								...
								if (a) {
									// Attack {0} ({1}) with {2} ({3}). (You can
									// use up to {4} dice to attack)
									output = RiskUtil
											.replaceAll(
													RiskUtil.replaceAll(
															RiskUtil.replaceAll(
									...
									throw new IllegalArgumentException(
											resb.getString("core.attack.error.unable"));
								}
							} else {
								throw new IllegalArgumentException(
										RiskUtil.replaceAll(
												resb.getString("core.error.syntax"),
												"{0}", "attack country country"));
							}
						} else if (input.equals("endattack")) {
							if (StringT.hasMoreTokens() == false) {
								if (game.endAttack()) {
									output = resb
											.getString("core.attack.end.ended");
								} else {
									throw new IllegalArgumentException(
											resb.getString("core.attack.end.error.unable"));
								}
							} else {
								throw new IllegalArgumentException(
										RiskUtil.replaceAll(
						...
						if (input.equals("roll")) {
							...
							if (StringT.countTokens() == 1) {
								...
								int dice = RiskGame.getNumber(StringT
										.nextToken());
								...
								if (dice != -1 && game.rollA(dice)) {
									...
									// Rolled attacking dice, {0} defend
									// yourself! (you can use up to {1} dice to
									// defend)
									output = RiskUtil
											.replaceAll(
													RiskUtil.replaceAll(
															resb.getString("core.roll.rolled"),
															"{0}",
															((Player) game
																	.getCurrentPlayer())
																	.getName()),
									...
									// ((Country)game.getDefender()).getOwner();
									//
									// if (
									...
									throw new IllegalArgumentException(
											resb.getString("core.roll.error.unable"));
								}
							} else {
								throw new IllegalArgumentException(
										RiskUtil.replaceAll(
							...
							if (StringT.hasMoreTokens() == false) {
								...
								if (game.retreat()) {
									output = resb
											.getString("core.retreat.retreated");
								} else {
									throw new IllegalArgumentException(
											resb.getString("core.retreat.error.unable"));
								}
							} else {
								throw new IllegalArgumentException(
										RiskUtil.replaceAll(
						...
						if (input.equals("move")) {
							if (StringT.countTokens() == 1) {
								...
								if (mov != 0) {
									// Moved {0} armies to captured country.
									output = RiskUtil.replaceAll(
											resb.getString("core.move.moved"),
											"{0}", "" + noa);
									...
									throw new IllegalArgumentException(
											resb.getString("core.move.error.unable"));
								}
							} else {
								throw new IllegalArgumentException(
										RiskUtil.replaceAll(
						...
						if (input.equals("movearmies")) {
							if (StringT.countTokens() == 3) {
								...
								int noa = RiskGame.getNumber(StringT
										.nextToken());
								...
								if (game.moveArmy(country1, country2, noa)) {
									...
									throw new IllegalArgumentException(
											resb.getString("core.tacmove.error.unable"));
								}
							} else {
								throw new IllegalArgumentException(
										RiskUtil.replaceAll(
							...
							if (StringT.hasMoreTokens() == false) {
								if (game.noMove()) {
									output = resb
											.getString("core.tacmove.no.nomoves");
								} else {
									throw new IllegalArgumentException(
											resb.getString("core.tacmove.no.error.unable"));
								}
							} else {
								throw new IllegalArgumentException(
										RiskUtil.replaceAll(
						...
						if (input.equals("endgo")) {
							if (StringT.hasMoreTokens() == false) {
								...
								controller.sendMessage(
										resb.getString("core.endgo.ended"),
										false, false);
								DoEndGo();
						...
						if (input.equals("continue")) {
							if (StringT.hasMoreTokens() == false) {
								...
								if (game.continuePlay()) {
									output = resb
											.getString("core.continue.successful");
								} else {
									throw new IllegalArgumentException(
											resb.getString("core.continue.error.unable"));
								}
							} else {
								throw new IllegalArgumentException(
										RiskUtil.replaceAll(
							...
							// The game is over. {0} won! (current possible
							// commands are: continue)
							throw new IllegalArgumentException(
									RiskUtil.replaceAll(resb
											.getString("core.gameover.won"),
											"{0}", game.getCurrentPlayer()
													.getName()));
						}
						...
						if (input.equals("capital")) {
							if (StringT.countTokens() == 1) {
								...
								if (nCountryId != -1) {
									...
									// YURA:LANG t = game.getCountryByName(
									// strCountry);
									t = null;
								}
								...
								if (t != null && game.setCapital(t)) {
									if (showHumanPlayerThereInfo()) {
										output = RiskUtil
												.replaceAll(
														resb.getString("core.capital.selected"),
														"{0}", t.getName()); // Display
									} else {
										output = resb
												.getString("core.capital.hasbeenselected");
									}
								} else {
									throw new IllegalArgumentException(
											resb.getString("core.capital.error.unable"));
								}
							} else {
								throw new IllegalArgumentException(
										RiskUtil.replaceAll(
						...
						if (input.equals("roll")) {
							...
							if (StringT.countTokens() == 1) {
								...
								if (dice != -1 && game.rollD(dice)) {
									...
									// client does a roll, and this is not
									// called
									if (shouldGameCommand(Addr)) { // recursive
																	// call

										int[] attackerResults = game
												.rollDice(game
														.getAttackerDice());
										int[] defenderResults = game
												.rollDice(game
														.getDefenderDice());
										...
										serverRoll = serverRoll
												+ attackerResults.length + " ";
										serverRoll = serverRoll
												+ defenderResults.length + " ";
						...
						throw new IllegalStateException(
								resb.getString("core.error.unknownstate"));
					}
				} catch (IllegalArgumentException ex) {
					if (aiPlayer) {
						logger.log(
								Level.WARNING,
								"bad command from ai: \"" + message
										+ "\" player="
		...
		if (game != null && game.getState() == RiskGame.STATE_END_TURN
				&& game.getCurrentPlayer().getAutoEndGo()) {
			needInput = false;
			DoEndGo();
		}
	...
	public void setMap(String filename) throws Exception {
		...
		if (game.getState() == RiskGame.STATE_NEW_GAME) {
			...
			// New map file selected: "{0}" (cards have been reset to the
			// default for this map)
			String output = RiskUtil.replaceAll(
					resb.getString("core.choosemap.mapselected"), "{0}",
					filename);
	...
	public void getMapError(String exception) {
		...
		String output = resb.getString("core.choosemap.error.unable") + " "
				+ exception;
		controller.sendMessage(output, false, true);
		showMessageDialog(output);
	}
	...
	/**
	 * return true ONLY if info of this Player p should be disclosed to this
	 * computer
	 */
	private boolean showHumanPlayerThereInfo(Player p) {
		return game.getState() == RiskGame.STATE_GAME_OVER
				|| ((p != null) && (p.getType() == Player.PLAYER_HUMAN) && (unlimitedLocalMode || myAddress
						.equals(p.getAddress())));
	}
	...
	public void getInput() {
		...
		// if we have more commands we need to process, do not bother asking for
		// input
		if (!inbox.isEmpty())
			return;
		...
		// work out what to do next
			...
			if (!replay) {
				...
				// else if (
				// game.getCurrentPlayer().getType()==Player.PLAYER_HUMAN ) {
	...
	public String whoWon() {
		...
		String text = System.getProperty("line.separator")
				+ RiskUtil.replaceAll(resb.getString("core.whowon.hehaswon"),
						"{0}", winner.getName());
		if (game.getGameMode() == RiskGame.MODE_SECRET_MISSION) {
			// There mission was: {0}
			text = text
					+ System.getProperty("line.separator")
					+ RiskUtil.replaceAll(
							resb.getString("core.whowon.mission"), "{0}",
							winner.getMission().getDiscription());
		}
		return text;
	}
	...
	public void setHelp() {
		...
		if (game != null && game.getCurrentPlayer() != null) {
			...
			try {
				help = RiskUtil.replaceAll(resb.getString(strId), "{0}", game
		...
		if (game == null) {
			...
			if (game.getSetupDone()) {
				help = help + resb.getString("core.help.placearmies");
			}

			else if (game.NoEmptyCountries()) {
				help = help + resb.getString("core.help.placearmy");
			}
			...
			help = RiskUtil.replaceAll(resb.getString("core.help.gameover"),
	...
	private void saveGameToUndoObject() {
		...
		if (unlimitedLocalMode) {
			...
			try {
				...
				// what can we do :-(
				Undo.reset(); // do not keep broken data, TODO, this does NOT
								// clean up memory
				skipUndo = true;
				logger.log(Level.INFO,
						resb.getString("core.loadgame.error.undo"), e);
			} catch (Throwable e) {
				Undo.reset(); // do not keep broken data, TODO, this does NOT
								// clean up memory
				skipUndo = true;
	...
	public void disconnected() {
		...
		controller.sendMessage(
				resb.getString("core.kicked.error.disconnected"), false, false);
	...
	private void updateBattleState() {
		if ((game.getState() == RiskGame.STATE_ROLLING || game.getState() == RiskGame.STATE_DEFEND_YOURSELF)
			...
			if (showHumanPlayerThereInfo(attackingPlayer)
					|| showHumanPlayerThereInfo(defendingPlayer)) {
				controller.openBattle(game.getAttacker().getColor(), game
						.getDefender().getColor());
				// we are opeing the battle at a strange point, when NODAttacker
				// is already set, so we should update it on the battle
				if (game.getState() == RiskGame.STATE_DEFEND_YOURSELF) {
	 ...
	 * 
	 * @param a
	 *            The name of the country attacking
	 * @param d
	 *            The name of the country defending
	 * @return boolean Returns true if the player owns the country, else returns
	 *         false
	 * 
	 *         // * @deprecated
	 * 
	 ...
	 * 
	 *         if ( ((Country)game.getCountry(a)).isNeighbours(
	 *         (Country)game.getCountry(d) ) ) { return true; } else { return
	 *         false; }
	 * 
	 *         }
	 */
	 ...
	 * @param nCountryTo
	 *            The name of the country defending
	 * @return boolean Returns true if the player can attack the other one,
	 *         false if not
	 */
	public boolean canAttack(int nCountryFrom, int nCountryTo) {
		if (game.getCountryInt(nCountryFrom).isNeighbours(
				game.getCountryInt(nCountryTo))) {
			return true;
		}
		return false;
	}// public boolean canAttack(int nCountryFrom, int nCountryTo)
	...
	public boolean isOwnedCurrentPlayerInt(int name) {
		...
		// not thread safe, so this can cause problems, but this method is used
		// in display to thats ok

		if ((game != null && game.getCurrentPlayer() != null && game
				.getCountryInt(name) != null)
				&&

				(((Country) game.getCountryInt(name)).getOwner() == null || ((Country) game
	...
	public String getCurrentMission() {
		...
		// }
			...
			return ((Mission) ((Player) game.getCurrentPlayer()).getMission())
					.getDiscription();
		} else {
	...
	public int[] getPlayerColors() {
		...
		// sometimes this method can get called if we close a game half way
		// through a paint
		if (g == null)
			return new int[0];
		...
		for (int c = 0; c < Players.size(); c++) {
			...
			if (((Player) Players.get(c)) == g.getCurrentPlayer()) {
				start = c;
			}
		}
	 ...
	 * @return String Return Country name if it is there, else return empty
	 *         speech-marks otherwise
	 */
	 ...
	 * 
	 *         public String getCountryDisplayName(int nCountryId) { Country
	 *         country = game.getCountryInt( nCountryId); if (country == null) {
	 *         return ""; } else { return country.getName(); // Display }
	...
	public boolean getAutoEndGo() {
		...
		if (game != null && game.getCurrentPlayer() != null) {
			...
			// this should never happen, but can come up with bad timing
			// problems
			return false;
		}
	}
	...
	private synchronized void closeGame() {
		...
		// if there are more commands for this game from the network we should
		// clear them
		if (!inbox.isEmpty()) {
	...
	public void newMemoryGame(RiskGame g, String map) {
		...
		controller.showCardsFile("loaded from memory",
				(game.getNoMissions() != 0));

		// we dont do this here as it wont work
		// controller.showMapPic(game);
		// the preview pic is instead set directly by the MapEditor calling on
		// the SetupPanel
	...
	public void setGame(RiskGame b) {
		...
		inbox.clear();
		game = b;
		controller.startGame(unlimitedLocalMode);// need to always call this as
													// there may be a new map
		getInput();
	}

	private void renamePlayer(String name, String newName, String newAddress,
			int newType) {

		if ("".equals(name) || "".equals(newName) || "".equals(newAddress)
				|| newType == -1) {
			throw new IllegalArgumentException("bad rename " + name + " "
		...
		if (leaver == null) {
			throw new IllegalArgumentException(
					"can not find player with name \"" + name + "\"");
		}
		if (newNamePlayer != null && !name.equals(newName)) {
			throw new IllegalArgumentException("can not rename \"" + name
					+ "\". someone with new name \"" + newName
