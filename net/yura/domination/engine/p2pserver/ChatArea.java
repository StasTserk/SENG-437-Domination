...
public class ChatArea extends Thread {
	...
	public ChatArea(RiskController g, int port) throws Exception {
		...
		gui.sendMessage("getHostAddress = " + iaddr.getHostAddress(), false,
				false);
	...
	public void run() {
		...
		try {
			...
			while (true) {// Forever loop
				...
				/*
				 * // this stops 2 clients joining from the same host boolean
				 * bad=false; for (int c=0; c < sockets.size();c++) {
				 * 
				 * if (nextSock.getInetAddress().equals(
				 * ((Socket)sockets.elementAt(c)).getInetAddress() )) {
				 * bad=true; break; }
				 * 
				 * } if (bad) {
				 * 
				 * nextSock.shutdownOutput(); nextSock.shutdownInput();
				 * nextSock.close();
				 * 
				 * continue;
				 * 
				...
				gui.sendMessage("Another Client has joined: " + nThreadCount,
						false, false);

				// chatArr[nThreadCount] = new Chat();

				chatArr[nThreadCount] = childThread = new ChatServerThread(
