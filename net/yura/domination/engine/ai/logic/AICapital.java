...
public class AICapital extends AIDomination {
	...
	protected List<Country> getBorder(GameState gs) {
		...
		int attack = (int) gs.commonThreat.attackValue
				/ (gs.orderedPlayers.size() + 1);
		...
		for (Iterator<Country> i = gs.capitals.iterator(); i.hasNext();) {
			...
			int additional = Math.max(attack - c.getArmies(),
					additionalTroopsNeeded(c, gs));
			if (additional <= 0) {
				return border;
			}
	...
	protected String planObjective(boolean attack, List<Country> attackable,
		...
		if (game.getSetupDone()) {
			...
			for (Iterator<Country> i = gameState.capitals.iterator(); i
					.hasNext();) {
				Country c = i.next();
			...
			// offensive planning
			if (highProbability
					|| (isIncreasingSet() && (ratio / 3 > percentOwned))
					|| (percentOwned >= .5 && (isIncreasingSet() || ratio > 1))) {
				String result = planCapitalMove(attack, attackable, gameState,
						targets, null, highProbability, allCountriesTaken,
						!highProbability, shouldEndAttack);
				if (result != null) {
					return result;
				}
			}
			// defensive planning
			for (Iterator<Map.Entry<Player, Integer>> i = owned.entrySet()
					.iterator(); i.hasNext();) {
				...
				for (Iterator<Country> j = gameState.capitals.iterator(); j
						.hasNext();) {
					Country c = j.next();
					if (c.getOwner() != other) {
				...
				for (int j = 0; j < gameState.orderedPlayers.size(); j++) {
					...
					if (ps.p == other) {
						if (gameState.commonThreat == null
								...
								&& ps.attackValue > (ps.strategic ? 3 : 4)
										* primaryDefense) {
						...
						if (ps.attackValue > 2 * primaryDefense) {
							if (myowned < 2) {
								// can we take one - TODO: coordinate with break
								// continent
								String result = planCapitalMove(attack,
										attackable, gameState, targets,
										e.getKey(), false, allCountriesTaken,
										!highProbability, shouldEndAttack);
								if (result != null) {
									return result;
								}
			...
			// defend the capital more - ensures that when playing with a small
			// number of players
	...
	/**
	 * Plans to take one (owned by the target player) or all of the remaining
	 * capitals.
	 * 
	...
	private String planCapitalMove(boolean attack, List<Country> attackable,
			GameState gameState, Map<Country, AttackTarget> targets,
			Player target, boolean allOrNone, Set<Country> allCountriesTaken,
			boolean lowProbability, boolean shouldEndAttack) {
		...
		if (!toAttack.isEmpty()) {
			if (allOrNone) {
				...
				return eliminate(attackable, targets, gameState, attack,
						remaining, allCountriesTaken, et, shouldEndAttack,
						lowProbability);
			}
			Collections.sort(toAttack, Collections.reverseOrder());
			AttackTarget at = toAttack.get(0);
			int route = findBestRoute(attackable, gameState, attack, null, at,
	...
	protected String getBattleWon(GameState gameState) {
		...
		if (gameState.capitals.contains(game.getAttacker())) {
			...
			return "move "
					+ Math.max(game.getMustMove(), -needed / 2
							- getMinPlacement());
		}
		if (gameState.capitals.contains(game.getDefender())
				&& (ownsNeighbours(player, game.getAttacker()) || !ownsNeighbours(
	...
	protected double getContinentValue(Continent co) {
		...
		if (!game.getSetupDone()) {
			// blunt the affect of continent modification so that we'll mostly
			// consider
