 ...
 * 
 *         TODO infer the other missions TODO modify isTooWeak and calculate the
 *         probable number of countries to take
 */
public class AIMission extends AIDomination {
	...
	protected String fortify(GameState gs, List<Country> attackable,
		...
		if (player.getMission().getNoofarmies() > 1
				&& (player.getTerritoriesOwned().size() - 3 >= player
						.getMission().getNoofcountries())) {
			String result = super.fortify(gs, attackable, true,
					player.getTerritoriesOwned());
			if (result != null) {
				return result;
			}
		}
	...
	public GameState getGameState(Player p, boolean excludeCards) {
		...
		if (player.getMission().getPlayer() != null && !isTargetMoot()
				&& player.getMission().getPlayer() != g.orderedPlayers.get(0).p) {
	...
	private boolean isTargetMoot() {
		return player.getMission().getPlayer() != null
				&& (player.getMission().getPlayer() == player || player
						.getMission().getPlayer().getTerritoriesOwned()
						.isEmpty());
	}
	...
	protected boolean shouldProactivelyFortify(Continent co, boolean attack,
		...
		boolean result = super.shouldProactivelyFortify(co, attack, attackable,
				gameState, targets, pressAttack, continents);
