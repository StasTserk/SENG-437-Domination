...
public class AITest extends AISubmissive {
	...
	public String getPlaceArmies() {
		...
		if (game.NoEmptyCountries() == false) {
			...
			if (name == null) {
				return "placearmies " + ((Country) t.elementAt(0)).getColor()
	...
	public String getAttack() {
		...
		/*
		 * // Extract method: findAttackableNeighbors() Vector n; for (int a=0;
		 * a< t.size() ; a++) { if ( ((Country)t.elementAt(a)).getArmies() > 1 )
		 * { n = ((Country)t.elementAt(a)).getNeighbours(); for (int b=0; b<
		 * n.size() ; b++) { if ( ((Country)n.elementAt(b)).getOwner() != player
		 * ) { outputs.add( "attack " + ((Country)t.elementAt(a)).getColor() +
		 * " " + ((Country)n.elementAt(b)).getColor() ); } } } }
		 */
		outputs = findAttackableNeighbors(player.getTerritoriesOwned(), 0);
		if (outputs.size() > 0) {
			move = (Attack) outputs.elementAt((int) Math.round(Math.random()
					* (outputs.size() - 1)));
			// System.out.println(player.getName() + ": "+ move.toString());
			// //TESTING
			return move.toString();
			// return (String)outputs.elementAt( (int)Math.round(Math.random() *
			// (outputs.size()-1) ) );
		}
		return "endattack";
	}
	...
	/**
	 * (moved here from AICrap/AISubmissive) Attempts to find the first
	 * territory that can be used to attack from
	 * 
	 * @param p
	 *            player object
	 * @return Sring name is a move to attack from any space they can (that has
	 *         less than 500 armies) else returns null
	 */
	public String findAttackableTerritory(Player p) {
		Vector countries = p.getTerritoriesOwned();

		for (int i = 0; i < countries.size(); i++) {
			Vector neighbors = ((Country) countries.elementAt(i))
					.getNeighbours();
			for (int j = 0; j < neighbors.size(); j++) {
				if (((Country) neighbors.elementAt(j)).getOwner() != p) {
					if ((p.getCapital() != null && ((Country) countries
							.elementAt(i)).getColor() != p.getCapital()
							.getColor())
							|| p.getCapital() == null)
	 ...
	 * @param t
	 *            Vector of teritories
	 * @param ratio
	 *            - threshold of attack to defence armies to filter out
			...
			if (source.getOwner() == player && source.getArmies() > 1) {
				...
				for (int b = 0; b < n.size(); b++) {
					...
					if (target.getOwner() != player
							&& ((double) (source.getArmies() / target
									.getArmies()) > ratio)) { // simplify logic
						// output.add( "attack " + source.getColor() + " " +
						// target.getColor() );
