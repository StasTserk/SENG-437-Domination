...
/**
 * THIS IS NOT A REAL AI, THIS IS WHAT A HUMAN PLAYER THAT HAS RESIGNED FROM A
 * GAME BECOMES SO THAT OTHER PLAYERS CAN CARRY ON PLAYING, THIS AI NEVER
 * ATTACKS ANYONE, JUST FOLLOWS RULES
 * 
 * @author Yura Mamyrin
 */
public class AISubmissive implements AI {
	...
	public String getPlaceArmies() {
		...
		return getPlaceCommand(randomCountry(player.getTerritoriesOwned()),
				player.getExtraArmies() / 3 + player.getExtraArmies() % 3);
	}
	...
	public String getCapital() {
		return "capital "
				+ randomCountry(player.getTerritoriesOwned()).getColor();
	}
	...
	protected String getPlaceCommand(Country country, int armies) {
		return "placearmies "
				+ country.getColor()
				+ " "
				+ (!game.getSetupDone() ? 1 : Math.max(1,
						Math.min(player.getExtraArmies(), armies)));
	}
