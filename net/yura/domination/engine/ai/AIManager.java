...
public class AIManager {
	...
	public AIManager() {
		Iterator<Class<AI>> providers = Service
				.providerClasses(AIManager.class);
		while (providers.hasNext()) {
			try {
				// each AIManager has its own instances of AI players so the
				// state does not leak
	...
	public String getOutput(RiskGame game, int type) {
		...
		if (usethisAI == null) {
			throw new IllegalArgumentException("can not find ai for type "
					+ type);
		}
		...
		default:
			throw new IllegalStateException("AI error: unknown state "
					+ game.getState());
		}
