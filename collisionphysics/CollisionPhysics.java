...
public class CollisionPhysics {
	 ...
	 */
	public static void pointIntersectsRectangleOuter(float pointX,
		...
		assert (pointX >= rectX1 + radius) && (pointX <= rectX2 - radius)
				&& (pointY >= rectY1 + radius) && (pointY <= rectY2 - radius) : "Point (with radius) is outside the rectangular container!";
	 ...
	 *            response. Otherwise, set collision time to infinity.
	 */
	public static void pointIntersectsLineVertical(float pointX, float pointY,
			float speedX, float speedY, float radius, float lineX,
			float timeLimit, CollisionResponse response) {
	 ...
	 */
	public static void pointIntersectsLineHorizontal(float pointX,
			float pointY, float speedX, float speedY, float radius,
	 ...
	 *            response. Otherwise, set collision time to infinity.
	 */
	public static void pointIntersectsCircleOuter(float pointX, float pointY,
			float speedX, float speedY, float radius, float outerCenterX,
			float outerCenterY, float outerRadius, float timeLimit,
		...
		float t = pointIntersectsCircleOuterDetection(pointX, pointY, speedX,
				speedY, radius, outerCenterX, outerCenterY, outerRadius);
		// Accept 0 < t <= timeLimit
		if (t > 0 && t <= timeLimit) {
			...
			pointIntersectsLineNormalResponse(pointX, pointY, speedX, speedY,
					outerCenterX, outerCenterY, impactX, impactY, response, t);
		}
	}
	 ...
	 */
	private static float pointIntersectsCircleOuterDetection(float pointX,
			float pointY, float speedX, float speedY, float radius,
	 ...
	 */
	private static void pointIntersectsLineNormalResponse(float pointX,
			float pointY, float speedX, float speedY, float lineNormalX1,
			float lineNormalY1, float lineNormalX2, float lineNormalY2,
		...
		double lineAngle = Math.atan2(lineNormalY2 - lineNormalY1, lineNormalX2
				- lineNormalX1);
	 ...
	 *            response. Otherwise, set collision time to infinity.
	 */
	public static void pointIntersectsPolygon(float pointX, float pointY,
			float speedX, float speedY, float radius, int[] polygonXs,
			int[] polygonYs, int numPoints, float timeLimit,
			CollisionResponse response) {
		...
		for (int segment = 0; segment < numPoints; segment++) {
			...
			CollisionPhysics.pointIntersectsLineSegmentNoEndPoints(pointX,
					pointY, speedX, speedY, radius, lineX1, lineY1, lineX2,
					lineY2, timeLimit, tempResponse);
			if (tempResponse.t < response.t) {
		...
		for (int i = 0; i < numPoints; i++) {
			CollisionPhysics.pointIntersectsPoint(pointX, pointY, speedX,
					speedY, radius, polygonXs[i], polygonYs[i], 0, timeLimit,
					tempResponse);
			if (tempResponse.t < response.t) {
				response.copy(tempResponse);
			}
	 ...
	 *            response. Otherwise, set collision time to infinity.
	 */
	public static void pointIntersectsLine(float pointX, float pointY,
			float speedX, float speedY, float radius, float lineX1,
			float lineY1, float lineX2, float lineY2, float timeLimit,
			CollisionResponse response) {
		...
		if (lineX1 == lineX2) { // Vertical line
			...
			pointIntersectsLineHorizontal(pointX, pointY, speedX, speedY,
					radius, lineY1, timeLimit, response);
			return;
		}
		...
		float t = pointIntersectsLineDetection(pointX, pointY, speedX, speedY,
				radius, lineX1, lineY1, lineX2, lineY2)[0];
		// Accept 0 < t <= timeLimit
		if (t > 0 && t <= timeLimit) {
			// Call helper method to compute the response in Response object
			pointIntersectsLineResponse(pointX, pointY, speedX, speedY, lineX1,
					lineY1, lineX2, lineY2, response, t);
		}
	}
	 ...
	 *         the line.
	 */
	private static float[] pointIntersectsLineDetection(float pointX,
			float pointY, float speedX, float speedY, float radius,
		...
		if (radius > 0) {
			...
			double rotatedY = rotate(pointX - lineX1, pointY - lineY1,
					lineAngle)[1];
			if (rotatedY > 0) {
	 ...
	 *            : the given detected collision time
	 */
	private static void pointIntersectsLineResponse(float pointX, float pointY,
			float speedX, float speedY, float lineX1, float lineY1,
			float lineX2, float lineY2, CollisionResponse response, float t) {
	 ...
	 *            response. Otherwise, set collision time to infinity.
	 */
	public static void pointIntersectsLineSegmentNoEndPoints(float pointX,
			float pointY, float speedX, float speedY, float radius,
			float lineX1, float lineY1, float lineX2, float lineY2,
			float timeLimit, CollisionResponse response) {
		...
		if (lineX1 == lineX2) { // Vertical line
			...
			if (!(impactY >= lineY1 && impactY <= lineY2 || impactY >= lineY2
					&& impactY <= lineY1)) {
			...
			if (!(impactX >= lineX1 && impactX <= lineX2 || impactX >= lineX2
					&& impactX <= lineX1)) {
				response.reset();
			}
			return;
		...
		// Call helper method to compute the collision time.
		float[] result = pointIntersectsLineDetection(pointX, pointY, speedX,
				speedY, radius, lineX1, lineY1, lineX2, lineY2);
		float t = result[0];
	 ...
	 *            response. Otherwise, set collision time to infinity.
	 */
	public static void pointIntersectsLineSegment(float pointX, float pointY,
			float speedX, float speedY, float radius, float lineX1,
			float lineY1, float lineX2, float lineY2, float timeLimit,
			CollisionResponse response) {
		...
		CollisionPhysics.pointIntersectsLineSegmentNoEndPoints(pointX, pointY,
				speedX, speedY, radius, lineX1, lineY1, lineX2, lineY2,
				timeLimit, tempResponse);
		if (tempResponse.t < response.t) {
			response.copy(tempResponse);
		}
		// Check the two end points (with radius = 0) for probable collision
		CollisionPhysics.pointIntersectsPoint(pointX, pointY, speedX, speedY,
				radius, lineX1, lineY1, 0, timeLimit, tempResponse);
		if (tempResponse.t < response.t) {
			response.copy(tempResponse);
		}
		CollisionPhysics.pointIntersectsPoint(pointX, pointY, speedX, speedY,
				radius, lineX2, lineY2, 0, timeLimit, tempResponse);
		if (tempResponse.t < response.t) {
			response.copy(tempResponse);
		}
	}
	 ...
	 */
	public static void pointIntersectsMovingPoint(float p1X, float p1Y,
			float p1SpeedX, float p1SpeedY, float p1Radius, float p2X,
			float p2Y, float p2SpeedX, float p2SpeedY, float p2Radius,
			float timeLimit, CollisionResponse p1Response,
			CollisionResponse p2Response) {
		...
		// Call helper method to compute the collision time t.
		float t = pointIntersectsMovingPointDetection(p1X, p1Y, p1SpeedX,
				p1SpeedY, p1Radius, p2X, p2Y, p2SpeedX, p2SpeedY, p2Radius);

		// Accept 0 < t <= timeLimit
		if (t > 0 && t <= timeLimit) {
			// Call helper method to compute the responses in the 2 Response
			// objects
			pointIntersectsMovingPointResponse(p1X, p1Y, p1SpeedX, p1SpeedY,
					p1Radius, p2X, p2Y, p2SpeedX, p2SpeedY, p2Radius,
	 ...
	 */
	private static float pointIntersectsMovingPointDetection(float p1X,
			float p1Y, float p1SpeedX, float p1SpeedY, float p1Radius,
	 ...
	 *            : the given detected collision time.
	 */
	private static void pointIntersectsMovingPointResponse(float p1X,
			float p1Y, float p1SpeedX, float p1SpeedY, float p1Radius,
			float p2X, float p2Y, float p2SpeedX, float p2SpeedY,
			float p2Radius, CollisionResponse p1Response,
			CollisionResponse p2Response, float t) {
		...
		double lineAngle = Math.atan2(p2ImpactY - p1ImpactY, p2ImpactX
				- p1ImpactX);
		...
		// momentum
		p1SpeedPAfter = (diffMass * p1SpeedP + 2.0 * p2Mass * p2SpeedP)
				/ sumMass;
		p2SpeedPAfter = (2.0 * p1Mass * p1SpeedP - diffMass * p2SpeedP)
				/ sumMass;
	 ...
	 *            response for p1. Otherwise, set collision time to infinity.
	 */
	public static void pointIntersectsPoint(float p1X, float p1Y,
			float p1SpeedX, float p1SpeedY, float p1Radius, float p2X,
			float p2Y, float p2Radius, float timeLimit,
			CollisionResponse p1Response) {
		...
		float t = pointIntersectsMovingPointDetection(p1X, p1Y, p1SpeedX,
				p1SpeedY, p1Radius, p2X, p2Y, 0f, 0f, p2Radius);

		// Accept 0 < t <= timeLimit
		if (t > 0 && t <= timeLimit) {
			// Call helper method to compute and return the response given
			// collision time t.
			pointIntersectsPointResponse(p1X, p1Y, p1SpeedX, p1SpeedY, p2X,
					p2Y, p1Response, t);
		}
	}
	 ...
	 *            : the given detected collision time.
	 */
	private static void pointIntersectsPointResponse(float p1X, float p1Y,
			float p1SpeedX, float p1SpeedY, float p2X, float p2Y,
