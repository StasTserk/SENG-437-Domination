...
public class Ball {
	...
	public Ball(float x, float y, float radius, float speed, float angleInDegree) {
		...
		this.speedY = (float) (-speed * (float) Math.sin(Math
				.toRadians(angleInDegree)));
		this.radius = radius;
	}
	 ...
	 * @param timeLimit
	 *            : upperbound of the time interval.
	 */
	public void intersect(int[] box, float timeLimit) {
		...
		CollisionPhysics.pointIntersectsRectangleOuter(x, y, speedX, speedY,
				radius, 0, 0, box[0], box[1], timeLimit, tempResponse);
	...
	public void intersect(Ball another, float timeLimit) {
		...
		CollisionPhysics.pointIntersectsMovingPoint(this.x, this.y,
				this.speedX, this.speedY, this.radius, another.x, another.y,
				another.speedX, another.speedY, another.radius, timeLimit,
				thisResponse, anotherResponse);
	 ...
	 * 
	 * @param time
	 *            : the earliest collision time detected in the system. If this
	 *            ball's earliestCollisionResponse.time equals to time, this
	 *            ball is the one that collided; otherwise, there is a collision
	 *            elsewhere.
	 */
	...
	/**
	 * Draw itself using the given graphics context. public void draw(Graphics
	 * g) { g.setColor(color); g.fillOval((int)(x - radius), (int)(y - radius),
